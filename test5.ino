...
const byte ROWS=5;
char keys[ROWS][COLS] = {
  {0x60,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x30,0x2D,0x3D,0xB2,0xD1},
  {0xB3,0x71,0x77,0x65,0x72,0x74,0x79,0x75,0x69,0x6F,0x70,0x5B,0x5D,0x5C,0x00},
  {0xC1,0x61,0x73,0x64,0x66,0x67,0x68,0x6A,0x6B,0x6C,0x3B,0x27,0xB0,0x00,0x00},
...
void loop() {
  ...
  if(digitalRead(col8)==HIGH&&IStatus==0){
    Keyboard.press(keys[0][14]);
    IStatus=1;
  }else if(digitalRead(col8)==HIGH&&IStatus==1){
    Keyboard.release(keys[0][14]);
    IStatus=0;
  }
